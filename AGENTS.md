...
- `claude/prompts/` — scoped build prompts for each rebuild phase, in order (`01-materials-catalog.md`, `02-materials-import-export.md`, `03-materials-import-fix-and-delete.md`, `04-materials-attribute-list-import.md`, `05-materials-tax-code-linkage.md`, `06-materials-labor-tax-override-ui.md`, `07-chrome-brand-color-separation.md`, `08-materials-io-tax-linkage-bulk-edit.md`, `09-labor-rates-and-engines.md`, `10-complexity-multipliers.md`, `11-recurring-fees-and-sma.md`, `12-nav-consolidation-and-ui-consistency.md`). Prompts 09-11 begin the pricing-catalog phase (Integrated Systems Commercial first): labor rates + quoted/service engines, complexity multipliers (hours-only), recurring fees + SMA. Prompt 12 consolidates the nav into two tabbed sections (Catalog = Materials/Recurring Fees/Catalog I/O + reserved Consumables; Rates = Labor Rates/Complexity) and normalizes page/table widths. Read the next-numbered prompt before starting the next phase of the rebuild. `claude/prompts/samples/` holds real fixture files (prior-build exports, plus the real Stripe `product_tax_codes.csv`) referenced by these prompts — use them as actual test data, not just as descriptions.
